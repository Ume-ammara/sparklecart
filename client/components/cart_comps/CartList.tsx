...
const CartList = ({ item }: CartListProps) => {
  ...
  const [openRemoveCart, setOpenopenRemoveCart] = useState(false);

  const { removeFromCart, isLoading, error, success } = useCartStore();

  const handleRemoveCart = async () => {
    console.log("ITEM REMOVED : ", item._id);
    await removeFromCart(item._id)
    setOpenopenRemoveCart(false)
  };
  ...
  return (
    <>
        ...
        <div className="flex flex-1 gap-4">
          ...
          <div className="flex flex-col justify-between">
            ...
            <Button
              ...
              onClick={() => setOpenopenRemoveCart(true)}
            >
...

      {
        <ActionButton
          open={openRemoveCart}
          onOpenChange={setOpenopenRemoveCart}
